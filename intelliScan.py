...
def main():
    ...
    #Resultlist contains a dict of lists, port number+ service is the key and list of vulns is the value
    tests = []
    resultDict = {}
    for hkey,host in scan.items():
        for pkey,port in host['tcp'].items():
            ...
            if (args.dumb is True):
                ...
                #Do query TI
                resultDict[str(hkey) + "_" +str(pkey)+"_"+svname] = queryTI(svname)
                #Do query explicit tests
                tests.append(dbhandler.getTests(str(pkey),port['name']))
